...
class CikadaMod(loader.Module):
    ...
    async def cikadacmd(self, message):
        ...
        if reply is not None:
            if reply.from_id is not None:
                ...
                await utils.answer(
                    message=message,
                    response="<b>🚫 Модуль иди нахуй не робит</b>"
                )
    ...
    async def cikadarmcmd(self, message):
        ...
        if reply is not None:
            await utils.answer(
                message=message,
                response="<b>всосал пидор с нерестом</b>"
            )
            ...
            try:
                ...
                await utils.answer(
                    message=message,
                    response="<b>💀 Я и так не говорю ем про плуониум</b>"
                )
