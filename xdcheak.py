#meta developer: Я НЕПРИВЕТЛИВ САМ ДЕЛАЛ ЕГО
...
class CikadaMod(loader.Module):
    ...
    async def clearbullcmd(self, message):
        """Никому не говорить про плутониум"""
        ...
        await utils.answer(
            message=message,
            response="<b>Больше не буду говорить что плутониум сила(</b>"
        )
    ...
    async def cikadatcmd(self, message):
        ...
        if not args:
            return await utils.answer(
                message=message,
                response="<b>🚫 Не указан жёсткий аргумент про плутониум</b>"
            )
        ...
        await utils.answer(
            message=message,
            response="<b>Аргумент про плутониум добавлена</b>"
        )
    
    async def cikadarcmd(self, message):
        """Вкинуть рандом плутониума"""
    ...
    async def cikadacmd(self, message):
        ...
        if reply is not None:
            if reply.from_id is not None:
                await utils.answer(
                    message=message,
                    response="<b>PLUTONIUM МОЩЬЬЬЬ</b>"
                )
